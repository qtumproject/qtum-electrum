...
class Ledger_Client():
    ...
    def perform_hw1_preflight(self):
        try:
            ...
            self.segwitSupported = (int(firmware[0]) >= 1 and int(firmware[1]) >= 1 and int(firmware[2]) >= 10) or (
            firmwareInfo['specialVersion'] == 0x20 and int(firmware[0]) == 1 and int(firmware[1]) == 0 and int(
                firmware[2]) >= 4)
...
class LedgerPlugin(HW_PluginBase):
    ...
    def get_btchip_device(self, device):
        ledger = False
        if (device.product_key[0] == 0x2581 and device.product_key[1] == 0x3b7c) or (
                device.product_key[0] == 0x2581 and device.product_key[1] == 0x4b7c) or (
            device.product_key[0] == 0x2c97):
            ledger = True
        dev = hid.device()
    ...
    def setup_device(self, device_info, wizard):
        ...
        # client.get_xpub('m')
        client.get_xpub("m/44'/88'")  # TODO replace by direct derivation once Nano S > 1.1
