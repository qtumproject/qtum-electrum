...
APPDIR="$BUILDDIR/qtum-electrum.AppDir"
...
(
    cd "$PROJECT_ROOT"
    pushd qtum_electrum/locale
    if ! which msgfmt > /dev/null 2>&1; then
        echo "Please install gettext"
        exit 1
    fi
    for i in ./locale/*; do
        dir="$PROJECT_ROOT/qtum_electrum/$i/LC_MESSAGES"
        mkdir -p $dir
...
"$python" -m pip install --cache-dir "$CACHEDIR/pip_cache" -r "$PROJECT_ROOT/requirements.txt"
"$python" -m pip install --cache-dir "$CACHEDIR/pip_cache" -r "$PROJECT_ROOT/requirements-fixed.txt"
"$python" -m pip install --cache-dir "$CACHEDIR/pip_cache" -r "$PROJECT_ROOT/requirements-binaries.txt"
...
cp "$PROJECT_ROOT/qtum-electrum.desktop" "$APPDIR/qtum-electrum.desktop"
cp "$PROJECT_ROOT/qtum_electrum/gui/icons/electrum.png" "$APPDIR/electrum.png"
