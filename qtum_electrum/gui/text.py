...
class ElectrumGui:
    ...
    def update_history(self):
        ...
        b = 0
        self.history = []
        for tx_hash, tx_mined_info, value, balance in self.wallet.get_history():
            if tx_mined_info.conf:
    ...
    def run_popup(self, title, items):
        return self.run_dialog(title, list(map(lambda x: {'type': 'button', 'label': x}, items)), interval=1,
                               y_pos=self.pos + 3)
