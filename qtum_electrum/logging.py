...
def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("qtum_electrum_log_*.log")), reverse=True)
    for f in files[keep:]:
        os.remove(str(f))
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"qtum_electrum_log_{timestamp}_{PID}.log"
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("qtum_electrum."):
        name = name[14:]
...
def configure_logging(config):
    ...
    _logger.info(f"Qtum Electrum version: {ELECTRUM_VERSION} - {GIT_REPO_URL}")
