...
from .util import bfh, bh2u, QtumException
    ...
    def func_wrapper(*args):
        ...
        while True:
            ...
            try:
                ...
                _logger.info('bip32 protect_against_invalid_ecpoint: skipping index')
                n += 1
...
def serialize_xprv(xtype, c, k, depth=0, fingerprint=b'\x00'*4,
    ...
    if not ecc.is_secret_within_curve_range(k):
        raise QtumException('Impossible xprv (not within curve order)')
...
class InvalidMasterKeyVersionBytes(QtumException): pass
...
def deserialize_xkey(xkey, prv, *, net=None):
    ...
    if len(xkey) != 78:
        raise QtumException('Invalid length for extended key: {}'
                               .format(len(xkey)))
    depth = xkey[4]
